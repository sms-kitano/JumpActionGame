package jp.techacademy.ayaka.kitano.jumpactiongame
...
class GameScreen(private val mGame: JumpActionGame) : ScreenAdapter() {
    companion object {
        ...
        val WORLD_WIDTH = 10f
        val WORLD_HEIGHT = 15 * 10    // 10画面分登れば終了
        val GUI_WIDTH = 320f
        val GUI_HEIGHT = 480f
    ...
    init {
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.ayaka.kitano.jumpactiongame")
    ...
    private fun createstage() {
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            //踏み台をプレイヤーがジャンプできる高さ以下の距離を空けつつ配置
    ...
    private fun updatePlaying(delta: Float){
        ...
        // Player
        //プレイヤーの座標が0.5以下になった場合はジャンプ（ゲーム開始時にジャンプさせるため）
        if (mPlayer.y <= 0.5f) {
            mPlayer.hitStep()
        }
